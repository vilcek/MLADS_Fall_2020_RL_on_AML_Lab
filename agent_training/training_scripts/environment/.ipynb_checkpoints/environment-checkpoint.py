...
class Env:
    ...
    def plot_state_img(self):
        ...
        for i in range(self.pa.num_res):
            images.append(self.machine.canvas[i, :, :])

            for j in range(self.pa.num_nw):
                job_slot = np.zeros((self.pa.time_horizon, self.pa.max_job_size))
                if self.job_slot.slot[j] is not None:  # fill in a block of work
                    job_slot[: self.job_slot.slot[j].len, :self.job_slot.slot[j].res_vec[i]] = 0.5
                images.append(job_slot)
                if j == self.pa.num_nw - 1:
                    skip_row += 1
        ...
        backlog_width = int(math.ceil(self.pa.backlog_size / float(self.pa.time_horizon)))
        backlog = np.zeros((self.pa.time_horizon, backlog_width))
        backlog[: int(self.job_backlog.curr_size / backlog_width), : backlog_width] = 0.5
        
        try:
            backlog[int(self.job_backlog.curr_size / backlog_width), : self.job_backlog.curr_size % backlog_width] = 0.5
        except:
            backlog[int(self.job_backlog.curr_size / backlog_width)-1, : self.job_backlog.curr_size % backlog_width] = 0.5
            
        images.append(backlog)

        extra_info = np.ones((self.pa.time_horizon, 1)) * \
                     self.extra_info.time_since_last_new_job / \
                     float(self.extra_info.max_tracking_time_since_last_job)
        images.append(extra_info)
        
        fill_1 = np.array([1] * images[0].shape[0] * 2).reshape(images[0].shape[0], 2)
        img_top = images[0]
        idx_top = range(1,int(len(images[:-2])/2))
        img_bot = images[int(len(images[:-2])/2)]
        idx_bot = range(int(len(images[:-2])/2)+1, len(images[:-2]))

        for i in idx_top:
            img_top = np.concatenate((img_top, fill_1, images[i]), axis=1)
        for i in idx_bot:
            img_bot = np.concatenate((img_bot, fill_1, images[i]), axis=1)

        fill2 = np.array([1] * 2 * img_top.shape[1]).reshape(2, img_top.shape[1])
        img = np.concatenate((img_top, fill2, img_bot), axis=0)

        fill_3 = np.array([1] * (images[0].shape[0] * 4 + 4)).reshape(images[0].shape[0] * 2 + 2, 2)
        img = np.concatenate((img, fill_3), axis=1)

        fill_4 = np.array([1] * int(images[-2].shape[0] / 2 * images[-2].shape[1])).reshape(int(images[-2].shape[0] / 2), images[-2].shape[1])
        fill_5 = np.array([1] * images[-2].shape[1]).reshape(1, images[-2].shape[1])
        img_right = np.concatenate((fill_4, fill_5, images[-2], fill_5), axis=0)
        img_right = np.concatenate((img_right, fill_4), axis=0)
...
class Machine:
    def __init__(self, pa):
        ...
        #self.colormap = np.arange(0.25, 0.75, 0.5 / float(pa.job_num_cap))
        self.colormap = np.concatenate((np.arange(0.2, 0.4, 0.4 / float(pa.job_num_cap)),
                                        np.arange(0.6, 0.8, 0.4 / float(pa.job_num_cap))))
